...
use mpi_utilities_mod,    only : my_task_id, broadcast_send, broadcast_recv,              & 
                                 ...
                                 broadcast_send_seqobs, broadcast_recv_seqobs
...
logical :: packed_sends = .false. ! unused in this code
...
namelist / assim_tools_nml / filter_kind, cutoff, sort_obs_inc, &
   ...
   packed_sends, sync_between_timers, detailed_timers, debug_mode, & 
...
!write(*,*) "Num special cutoffs = ", num_special_cutoff
...
real(r8) :: checksum, checksum_total
...
!SEQUENTIAL_OBS: do i = 1, 2
   !write(*,*) "----------------------------------"

        !if (is_doing_vertical_conversion) then
        !      write(*,*)" VERTICAL CONVERSION is TRUE"
        !else
        !       write(*,*) "VERTICAL CONVERSION is false"
        !endif

!#ifdef TIMER_SYNC
   !call task_sync()
!#endif
   !call t_startf('Loop.Header')

   !write(*,*) "DEBUG: ", obs_ens_handle%num_vars, obs_ens_handle%my_num_vars

   if (sync_between_timers) then
     call task_sync() !-- filter3
   endif
   call t_startf('ASSIMILATE:Loop(Header)')
   ...
   !write(*,*) "DEBUG: get_obs_from_key : ", keys(i)
   ...
   call t_stopf('ASSIMILATE:Loop(Header)')
  ...
  !write(*,*) "DEBUG: map_pe_to_task(ens_handle, owner) = ", map_pe_to_task(ens_handle, owner), owner
   ...
   if(ens_handle%my_pe == owner) then
      ...
      IF_QC_IS_OKAY: if(nint(obs_qc) ==0) then
         ...
         !write(*,*) "DEBUG : obs_prior_sum = ", sum(obs_prior), i
      ...
      if(local_varying_ss_inflate) then
          !write(*,*) " local_varying_ss_inflate is true"
         call broadcast_send_seqobs(map_pe_to_task(ens_handle, owner), obs_prior, obs_inc, &
         ...
         !write(*,*) " local_single_ss_inflate or local_obs_inflate is true"
         call broadcast_send_seqobs(map_pe_to_task(ens_handle, owner), obs_prior, obs_inc, &
         ...
         !write(*,*) " local_single_ss_inflate or local_obs_inflate is FALSE"
         call broadcast_send_seqobs(map_pe_to_task(ens_handle, owner), obs_prior, obs_inc, &
   ...
   !-----------------------------------------------------------------------
      ...
      if(local_varying_ss_inflate) then
         call broadcast_recv_seqobs(map_pe_to_task(ens_handle, owner), obs_prior, obs_inc, &
            orig_obs_prior_mean, orig_obs_prior_var, net_a, scalar1=obs_qc, &
            scalar2=vertvalue_obs_in_localization_coord, scalar3=whichvert_real)
      else if(local_single_ss_inflate .or. local_obs_inflate) then
         call broadcast_recv_seqobs(map_pe_to_task(ens_handle, owner), obs_prior, obs_inc, &
...
!end do
  ...
  if (sync_between_timers) then
   ...
   do group = 1, num_groups
      ...
      !write(*,*) "DEBUG : obs_prior_mean = ", obs_prior_mean(group), i
   end do
   !write(*,*) "DEBUG: Priors = ", i, sum(obs_prior_mean), sum(obs_prior_var)
   ...
   if (.not. close_obs_caching) then
      !write(*,*) "Close obs caching is FALSE"
      if (timing) call start_mpi_timer(base)
      ...
      !write(*,*) "Close obs caching is TRUE"
      ...
      if (base_obs_loc == last_base_obs_loc) then
         ...
         !write(*,*) "Entering get_close_obs : ", i, base_obs_loc%lon, base_obs_loc%lat
         call get_close_obs(gc_obs, base_obs_loc, base_obs_type, &
                            my_obs_loc, my_obs_kind, my_obs_type, &
                            num_close_obs, close_obs_ind, close_obs_dist, ens_handle)
   ...
   if(adaptive_localization_threshold > 0) then
      ...
      !if (my_task_id() == 0) then
      !  write(*,*) "DEBUG (Count) : ", i, total_num_close_obs, num_close_obs
      !endif
      !rev_close_ranks = count_ranks_with_close(num_close_obs, close_obs_ind, my_obs_type, &
      !                                  close_obs_dist, cutoff_rev*2.0_r8)
      ...
      if(total_num_close_obs > adaptive_localization_threshold) then
         ...
         if ( output_localization_diagnostics ) then
            ...
            ! bpd6
            !!write(*,*) "DEBUG (Brian) : ", i, size(close_obs_ind), total_num_close_obs, rev_num_close_obs
            !!call list_close(i, list_unit, rev_num_close_obs, num_close_obs, close_obs_ind, my_obs_type, &   
            !!                close_obs_dist, cutoff_rev*2.0_r8)
            ...
            if (my_task_id() == 0) then
               ...
               write(histogram_unit, '(A,i8,2x,A,i10,2x,A,i10)') "Obs: ", i, &
                     "Total: ", total_close_ranks, "Revised: ", rev_close_ranks
            endif

            !bpd6
            total_num_close_obs = rev_num_close_obs
         endif
         ...
         if ( output_localization_diagnostics ) then
            ...
            ! bpd6
            !!write(*,*) "DEBUG (Brian2) : ", i, size(close_obs_ind), total_num_close_obs, num_close_obs
            !!if (total_num_close_obs .NE. num_close_obs) then
            !!  !write(*,*) "DEBUG (Obs) : ", i, total_num_close_obs, num_close_obs
            !!endif
            !!call list_close(i, list_unit, total_num_close_obs, num_close_obs, close_obs_ind, my_obs_type, &    
            !!                close_obs_dist, cutoff_rev*2.0_r8)
            !!total_num_close_obs = count_close(num_close_obs, close_obs_ind, my_obs_type, &
            !!                            close_obs_dist, cutoff_rev*2.0_r8)
            !write(*,*) "DEBUG (Two) : ", i, total_num_close_obs, num_close_obs
         endif
      endif
      ...
      total_num_close_obs = count_close(num_close_obs, close_obs_ind, my_obs_type, &
                                        close_obs_dist, cutoff_rev*2.0_r8)
      !total_close_ranks = count_ranks_with_close(num_close_obs, close_obs_ind, my_obs_type, &
      !                                  close_obs_dist, cutoff_rev*2.0_r8)
      ...
      if (my_task_id() == 0) then
         ...
         write(histogram_unit, '(A,i8,2x,A,i10)') "Obs: ", i, "Total: ", total_close_ranks
      endif
   endif
...
!   num_close_states = 0
   if (.not. close_obs_caching) then
      ...
      if (base_obs_loc == last_base_states_loc) then
         !call t_startf('seqobs_D_getclosestate_cache')
         !write(*,*) "Debug : base_obs_loc == last_base_states_loc"
         ...
         !call t_stopf('seqobs_D_getclosestate_cache')
      else
         !call t_startf('seqobs_D_getclosestate_sec')
         if (timing .and. i < 100) call start_mpi_timer(base)
         call get_close_state(gc_state, base_obs_loc, base_obs_type, &
                              my_state_loc, my_state_kind, my_state_indx, &
                              num_close_states, close_state_ind, close_state_dist, ens_handle)
         ...
         !call t_stopf('seqobs_D_getclosestate_sec')
      endif
   endif
...
! debug stuff:
  ...
  !if (my_task_id() == 0) then
  !  write(*,'(A,I,I,A,F,I)') "DEBUG: Close States / Obs : ", num_close_states_total, num_close_obs_total, "     ", checksum_total, i

  !endif

!   num_close_states = 0

!bpd6 - keep commented, just for understanding above call
!      call get_close_obs(gc_obs, base_obs_loc, base_obs_type, &
!                         my_obs_loc, my_obs_kind, my_obs_type, &
!                         num_close_obs, close_obs_ind, close_obs_dist, ens_handle)
   !bpd6
!   if ( output_localization_diagnostics ) then
!     call list_close      (i, list_unit, total_num_close_obs, num_close_obs, close_obs_ind, my_obs_type, close_obs_dist, cutoff_rev*2.0_r8)
!     !call list_close_state(i, list_unit, num_close_states, close_state_ind)
!   else
!     !write(list_unit,'(A)') ""
!   endif
...
skipped_missing = 0
skipped_covfactor = 0
stateupdate_time = 0.0

 ! start the timer for this section for this rank:
  call system_clock(count=timer_count, count_rate=timer_rate)

   !write(*,*) "DEBUG : num_close_states = ", num_close_states, i 
   !write(*,*) "close_state_dist(1) = ", close_state_dist(1), i
   !write(*,*) "cutoff_rev = ", cutoff_rev, i
   !write(*,*) "my_state_loc(1) = ", my_state_loc(close_state_ind(1))%lon, i
...
!PAR NEED TO TURN STUFF OFF MORE EFFICEINTLY
    ...
    !write(*,*) "DEBUG : cov_factor sum = ", testval, i
    !write(*,*) "DEBUG:  sum_increment = ", testval2, i
...
!  write(*,*) "Debug: ", stateupdate_time
   ...
   !write(*,*) "num_close_obs = ", num_close_obs, i
   ...
   OBS_UPDATE: do j = 1, num_close_obs
      ...
      !write(*,*) "Obs Update Checksum : ", sum(obs_prior), my_obs_indx(obs_index), i
...
!write(*,*) "Exiting..." ! Temporary hack - bpd6
...
call t_startf('update_from_obs_inc')
...
!  write(*,*) "SR1 : ", reg_coef, obs_inc
!else
!  write(*,*) "SR0 : ", reg_coef, obs_inc
!endif
...
call t_stopf('update_from_obs_inc')
