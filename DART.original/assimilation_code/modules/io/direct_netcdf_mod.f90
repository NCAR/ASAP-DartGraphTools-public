...
!
! $Id: direct_netcdf_mod.f90 11864 2017-08-02 19:40:28Z nancy@ucar.edu $
...
use mpi_utilities_mod,    only : task_count, io_send_to, io_receive_from, my_task_id, &
                                 broadcast_flag
...
character(len=32 ), parameter :: revision = "$Revision: 11864 $"
character(len=128), parameter :: revdate  = "$Date: 2017-08-02 13:40:28 -0600 (Wed, 02 Aug 2017) $"
...
call t_startf('IO:read_transpose')
...
call t_stopf('IO:read_transpose')
...
call t_startf('IO:write_transpose')
if (task_count() == 1) then
...
call t_stopf('IO:write_transpose')
...
!bpd6:
if (my_task_id() == 0) then
  write(*,*) "IO Info: ", ens_size, num_state_variables
endif
...
if (state_ens_handle%distribution_type == 1) then
   ...
   call io_send_to(map_pe_to_task(state_ens_handle, recv_pe), buffer)
   deallocate(buffer)
...
if (state_ens_handle%distribution_type == 1) then
   ...
   ! MPI vector data type or packing should be used here.
   allocate(buffer(finish - start + 1))
   call io_receive_from(map_pe_to_task(state_ens_handle, recv_pe), buffer)
...
if (state_ens_handle%distribution_type == 1) then
   ...
   ! MPI vector data type or packing should be used here.
   allocate(buffer(elm_count))
   call io_receive_from(map_pe_to_task(state_ens_handle, sending_pe), &
                           buffer)
