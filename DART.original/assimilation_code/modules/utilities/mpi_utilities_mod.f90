...
!
! $Id: mpi_utilities_mod.f90 11799 2017-07-07 21:08:09Z nancy@ucar.edu $
...
public :: initialize_mpi_utilities, finalize_mpi_utilities,                  &
          ...
          broadcast_state, broadcast_send_state, broadcast_recv_state, &
          broadcast_send_seqobs, broadcast_recv_seqobs, &
          io_send_to, io_receive_from, &
...
! bpd6
integer, parameter :: PACKLIMIT2 = 64
...
!integer, parameter :: BCAST_MAXSIZE = 1 * 50 * 1000 * 1000
...
call t_startf('MPI:send_to') ! GPTL
...
call t_stopf('MPI:send_to') ! GPTL

end subroutine send_to


! IO version of send_to used in direct_netcdf_mod
subroutine io_send_to(dest_id, srcarray, time, label) 
 integer, intent(in) :: dest_id
 real(r8), intent(in) :: srcarray(:)
 type(time_type), intent(in), optional :: time
 character(len=*), intent(in), optional :: label
...
call t_startf('IO:io_send_to') ! GPTL

if (verbose) write(*,*) "PE", myrank, ": start of io_send_to "

if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'io_send_to', errstring, source, revision, revdate)
endif
...
if (present(label)) then
   write(*,*) trim(label)//" PE", myrank, ": io_send_to itemsize ", itemcount, " dest ", dest_id
else if (verbose) then
   write(*,*) "PE", myrank, ": io_send_to itemsize ", itemcount, " dest ", dest_id
endif
...
if (make_copy_before_sendrecv) then
   ...
   if (verbose) write(*,*) "PE", myrank, ": io_send_to alloctmp ", itemcount, " dest ", dest_id
   call MPI_Ssend(tmpdata, itemcount, datasize, dest_id, tag, &
                 my_local_comm, errcode)
else
   ! there are a few places in the code where we send/receive a full state vector.
   ...
   if (itemcount > SNDRCV_MAXSIZE) then
      ...
      if (verbose) write(*,*) "PE", myrank, ": io_send_to ", itemcount, " dest ", dest_id
      call MPI_Ssend(srcarray, size(srcarray), datasize, dest_id, tag, &
                    my_local_comm, errcode)
   endif
endif
...
if (verbose) write(*,*) "PE", myrank, ": end of io_send_to "

call t_stopf('IO:io_send_to') ! GPTL

end subroutine io_send_to
...
call t_startf('MPI:receive_from') ! GPTL
...
call t_stopf('MPI:receive_from') ! GPTL

end subroutine receive_from


subroutine io_receive_from(src_id, destarray, time, label)
 integer, intent(in) :: src_id
 real(r8), intent(inout) :: destarray(:)
 type(time_type), intent(out), optional :: time
 character(len=*), intent(in), optional :: label
...
call t_startf('IO:io_receive_from') ! GPTL

if (verbose) write(*,*) "PE", myrank, ": start of io_receive_from "

if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'io_receive_from', errstring, source, revision, revdate)
endif
...
if (present(label)) then
   write(*,*) trim(label)//" PE", myrank, ": io_receive_from itemsize ", itemcount, " src ", src_id
else if (verbose) then
   write(*,*) "PE", myrank, ": io_receive_from itemsize ", itemcount, " src ", src_id
endif
...
if (make_copy_before_sendrecv) then
   ...
   if (verbose) write(*,*) "PE", myrank, ": io_receive_from alloctmp ", itemcount, " src ", src_id
   ...
   if (itemcount > SNDRCV_MAXSIZE) then
      ...
      if (verbose) write(*,*) "PE", myrank, ": io_receive_from ", itemcount,  " src ", src_id
      call MPI_Recv(destarray, itemcount, datasize, src_id, MPI_ANY_TAG, &
                 my_local_comm, status, errcode)
   endif
...
if (present(time)) then
   ...
   if (itime(2) < 0) then
      write(errstring, '(a,i8)') 'seconds in settime were < 0; using 0'
      call error_handler(E_MSG,'io_receive_from', errstring, source, revision, revdate)
      time = set_time(itime(1), 0)
   else
      time = set_time(itime(1), itime(2))
   endif
...
if (verbose) write(*,*) "PE", myrank, ": end of io_receive_from "

call t_stopf('IO:io_receive_from') ! GPTL

end subroutine io_receive_from
...
call t_startf('MPI:array_broadcast') ! GPTL
...
call t_stopf('MPI:array_broadcast') ! GPTL

end subroutine array_broadcast

subroutine broadcast_state(array, root)
 real(r8), intent(inout) :: array(:)
 integer, intent(in) :: root
...
call t_startf('MPI:broadcast_state') ! GPTL

if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'broadcast_state', errstring, source, revision, revdate)
endif
...
if (itemcount > BCAST_MAXSIZE) then
   ...
   !bpd6
   if (myrank == 0) then
      write(*,*) "Size, count, datasize : ", size(array), itemcount, datasize
   endif
   call MPI_Bcast(array, itemcount, datasize, root, my_local_comm, errcode)
   if (errcode /= MPI_SUCCESS) then
      write(errstring, '(a,i8)') 'MPI_Bcast returned error code ', errcode
...
call t_stopf('MPI:broadcast_state') ! GPTL
...
call t_startf('MPI:array_distribute') ! GPTL
...
call t_stopf('MPI:array_distribute') ! GPTL
...
subroutine broadcast_send_state(from, array1, array2, array3, array4, array5, &
                          scalar1, scalar2, scalar3, scalar4, scalar5)
 integer, intent(in) :: from
 ! really only intent(in) here, but must match array_broadcast() call.
 real(r8), intent(inout) :: array1(:)
...
if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'broadcast_send_state', errstring, source, revision, revdate)
endif
...
if (itemcount <= PACKLIMIT1 .and. morethanone) then
   ...
   call broadcast_state(packbuf1, from)
   ...
   call broadcast_state(packbuf2, from)

else

   call broadcast_state(array1, from)

   if (morethanone) then
      if (present(array2)) call broadcast_state(array2, from)
      if (present(array3)) call broadcast_state(array3, from)
      if (present(array4)) call broadcast_state(array4, from)
      if (present(array5)) call broadcast_state(array5, from)
      
      if (doscalar) then
         call packscalar(local, scalar1, scalar2, scalar3, scalar4, scalar5)
         call broadcast_state(local, from)
      endif

   endif
endif

end subroutine broadcast_send_state



subroutine broadcast_send_seqobs(from, array1, array2, array3, array4, array5, &
                          scalar1, scalar2, scalar3, scalar4, scalar5)
 integer, intent(in) :: from
 ! really only intent(in) here, but must match array_broadcast() call.
 real(r8), intent(inout) :: array1(:)
...
call t_startf('MPI:broadcast_send_seqobs')

if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'broadcast_send_seqobs', errstring, source, revision, revdate)
endif
...
call t_stopf('MPI:broadcast_send_seqobs')

end subroutine broadcast_send_seqobs
...
subroutine broadcast_recv_state(from, array1, array2, array3, array4, array5, &
                          scalar1, scalar2, scalar3, scalar4, scalar5)
 integer, intent(in) :: from
 ! really only intent(out) here, but must match array_broadcast() call.
 real(r8), intent(inout) :: array1(:)
...
if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'broadcast_recv_state', errstring, source, revision, revdate)
endif
...
end subroutine broadcast_recv_state

subroutine broadcast_recv_seqobs(from, array1, array2, array3, array4, array5, &
                          scalar1, scalar2, scalar3, scalar4, scalar5)
 integer, intent(in) :: from
 ! really only intent(out) here, but must match array_broadcast() call.
 real(r8), intent(inout) :: array1(:)
...
call t_startf('MPI:broadcast_recv_obs') ! GPTL

if ( .not. module_initialized ) then
   write(errstring, *) 'initialize_mpi_utilities() must be called first'
   call error_handler(E_ERR,'broadcast_recv_seqobs', errstring, source, revision, revdate)
endif
...
call t_stopf('MPI:broadcast_recv_obs') ! GPTL

end subroutine broadcast_recv_seqobs
