...
!#endif
   ...
   ! Debug - check key(i) vs i, per issue with coloring and discussion with
   ! Nancy
   !write(*,*) "ObID : ", i, keys(i)
...
!  write(*,*) "Debug: ", stateupdate_time
   ...
   OBS_UPDATE: do j = 1, num_close_obs
      ...
      !  write(*,*) "CloseCheck(1) : ", j, my_obs_indx(obs_index), i
      !endif
      ...
      if(my_obs_indx(obs_index) > i) then
         ...
         !if (any(obs_ens_handle%copies(1:ens_size, obs_index) == MISSING_R8)) then
         !  write(*,*) "CloseCheck(2) : ", j, "missing data (cycle)"
         !endif
         ...
         !write(*,*) "cov_factor (pre impact table) : ", cov_factor, close_obs_dist(j), cutoff_rev
         ...
         !if (cov_factor <= 0.0_r8) then
         !  write(*,*) "CloseCheck(3) : ", j, "cov_factor (cycle)", impact_factor
         !endif
