...
use      obs_def_mod, only : obs_def_type, get_obs_def_time, get_obs_def_type_of_obs,     &
                             ...
                             set_obs_def_time, get_obs_def_key
...
use obs_sequence_mod, only : obs_sequence_type, obs_type, write_obs_seq,       &
                             ...
                             get_obs_def, set_obs_def, get_qc, set_qc
...
! bpd6
integer :: obindex = 0
integer, dimension(:), allocatable :: extractlist 
real(r8), dimension(1) :: qc_value
...
call ReadExtractList(extractlist)
...
!      !write(*,*) "DEBUG:  Index = ", obindex
!      if (extractlist(obindex) == 1) then
...
!      endif
  ...
  if (mod(obindex, 2) == 0) then
     ...
     num_inserted = num_inserted + 1
      ...
      !if (extractlist(obindex) == 0) then
        ! This ob needs to be ignored; set qc to bad value:
!        call get_qc(obs_out, qc_value)
!        write(*,*) "QC : ", qc_value(1)
...
contains
!
!subroutine ReadColormap(colormap)
!   integer, dimension(:,:), allocatable, intent(out) :: colormap
!
!   integer :: numobs
!
!   ! Open the file:
!   open(unit=11, file='colormap.txt', status='old', readonly)
!
!   ! First line is the number of obs:
!   read(11, *) numobs
!
!  ! Allocate the array
!  allocate(colormap(numobs, 2))
!
!  do i=1,numobs
!      read(11, *) colormap(i,1), colormap(i,2)
!  enddo 
!
!  write(*,*) "Read in colormap: ", numobs, colormap(1,1), colormap(1,2)
!  close(11)
!
!end subroutine ReadColormap
!
!subroutine ReadColormap2(colormap)
!   integer, dimension(:,:), allocatable, intent(out) :: colormap
!
!   integer :: numobs,numnodes
!   integer :: node, color
!
!   ! Open the file:
!   open(unit=11, file='colormap.txt', status='old', readonly)
...
!  do i=1,numnodes
!      read(11, *) node,color
!      colormap(node,1) = node
!      colormap(node,2) = color
!  enddo 
!
!  write(*,*) "Read in colormap: ", numobs, numnodes
!  close(11)
!
!end subroutine ReadColormap2

subroutine ReadExtractList(extractlist)
   integer, dimension(:), allocatable, intent(out) :: extractlist

   integer :: numentries,numobs
   integer :: id

   ! Open the file:
   open(unit=11, file='colormap.txt', status='old', readonly)

   ! First line is the number of listed obs, total obs:
   read(11, *) numentries,numobs

  ! Allocate the array
  allocate(extractlist(numobs))
  extractlist = 0
  ...
  write(*,*) "Read in colormap: ", numentries,numobs
  close(11)

end subroutine ReadExtractList
